...
## #include <stdio.h>
- header file to access library. (Allows code such as the printf functionality). Stands for standard io.
...
## for printf:
- This function prints a “formatted string” to the screen. 
- It expects as input a “format string” that specifies what to print and zero or more subsequent arguments. 
- The format string can optionally contain “conversion specifications,” placeholders that begin with % that specify how to format the function’s subsequent arguments, if any.

### printf Examples
- For instance, if c is a char, this function can print it as follows using %c:
> printf("%c\n", c);
...
To specify the “precision” of a float or a double, %f can optionally contain a . after the % followed by a number of decimal places. 
- For instance, this function could format the value of one third to one decimal place using %.1f:
> printf("%.1f\n", 1.0 / 3.0);
...
### Other types of values (data types)
...
### Introducing concept conditionals: 
...
```
- = means assigns a return value from right-hand side of a line code to the left-hand side.
== means equals to.
...
What's y? 1
```
- This gives NO OUTPUT (only have if x < y)
```
#include <cs50.h>
#include <stdio.h>
...
```
- This is the best code.- To condense it even more we use a FOR LOOP:
```
for (int i = 0; i < 3; i++)
{
    printf("meow\n");
...
- To overcome this write integer as a LONG INT. 
- int -> long int. %i -> %li.
...
```
- If i want to do x:3 y:2 it should be 1.5. 
- But it outputs 1. 
- This is because of a truncation. 
- Int + int = int.
```
#include <cs50.h>
#include <stdio.h>
...
And it turns out, if you count the number of seconds between 1970 on up, on the day January 19, 2038, the world might again end because all of these clocks are going to overflow. 
