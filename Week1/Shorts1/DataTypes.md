...
- takes up 4 bytes memory (32 bits)
- Range of float is complicated 
- With 32 bits of precision, some of which might be used for an integer part but limited to how precicse we can be.

## double
- Used for variables that will store floating-point values, also known as real numbers.
- Real numbers are any numbers that can be found on a number line, including both rational and irrational numbers. 
- The difference is that doubles are double precision. 
- They always take up 8 bytes of memory (64 bits). With an additional 32 bits of precision relative to a float, doubles allow us to be specify much more precise real numbers.
...
## bool
- Used for variables that will store a boolean value. 
- Capable of storing one of two values, true and false.
- %d format specifier to print a boolean value:
